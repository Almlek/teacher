...
function normalizeQuestion(question: Partial<GeneratedExamQuestion>, index: number): GeneratedExamQuestion {
  const type = QUESTION_TYPES.includes(question.questionType as ExamQuestionType)
...
قواعد مهمة:
- استخدم أسئلة متنوعة، مع الالتزام بعدد الأسئلة المطلوب قدر الإمكان.
...
Rules:
- Use varied questions while keeping the requested count.
...
${source}`;
  ...
  const questions = Array.isArray(parsed.questions)
    ? parsed.questions.slice(0, params.questionCount).map((question, index) => normalizeQuestion(question, index))
    : [];
