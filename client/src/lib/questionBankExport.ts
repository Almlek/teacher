export type QuestionBankPdfBranding = {
  header?: string | null;
  logoUrl?: string | null;
};
...
export function buildQuestionBankPrintDocument(items: QuestionBankExportItem[], title = "قائمة الأسئلة المصفاة", branding: QuestionBankPdfBranding = {}) {
  ...
  const header = branding.header?.trim() || "";
  const logo = branding.logoUrl ? `<img class="school-logo" src="${escapeHtml(branding.logoUrl)}" alt="شعار المدرسة" />` : "";
  const brandingMarkup = header || logo ? `<div class="school-branding">${logo}<div class="school-header">${escapeHtml(header).replace(/\r?\n/g, "<br />")}</div></div>` : "";
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>@page{size:A4;margin:16mm}body{direction:rtl;font-family:Tahoma,Arial,sans-serif;color:#172033;line-height:1.8;margin:0}.sheet{max-width:180mm;margin:auto}.school-branding{display:flex;align-items:center;justify-content:center;gap:14px;border-bottom:2px solid #2563eb;padding:4px 0 12px;margin-bottom:16px;text-align:center}.school-logo{width:25mm;height:25mm;object-fit:contain}.school-header{font-weight:800;font-size:16px;color:#1e3a8a;white-space:normal}.header{text-align:center;border:2px solid #2563eb;border-radius:12px;padding:16px;margin-bottom:18px}h1{margin:0;color:#1d4ed8;font-size:23px}.summary{margin-top:6px;color:#64748b;font-size:12px}.question{border-bottom:1px solid #cbd5e1;padding:12px 0 16px;page-break-inside:avoid}.question h2{font-size:15px;margin:0 0 4px;color:#1e3a8a}.question h2 span{font-size:11px;color:#64748b}.meta,.tags{font-size:11px;color:#64748b}.options{white-space:pre-wrap;margin:7px 0}.question img{display:block;max-width:140mm;max-height:65mm;margin:10px auto;border:1px solid #dbeafe;border-radius:8px;object-fit:contain}.footer{text-align:center;margin-top:20px;border-top:1px solid #cbd5e1;padding-top:8px;font-size:11px;color:#64748b}</style></head><body><div class="sheet">${brandingMarkup}<header class="header"><h1>${escapeHtml(title)}</h1><div class="summary">عدد الأسئلة: ${items.length}</div></header>${questions || "<p>لا توجد أسئلة مطابقة للتصدير.</p>"}<div class="footer">تم التصدير من دفتر التحضير الذكي</div></div></body></html>`;
}

export function printQuestionBankAsPdf(items: QuestionBankExportItem[], title = "قائمة الأسئلة المصفاة", openWindow: typeof window.open = window.open, branding: QuestionBankPdfBranding = {}) {
  ...
  popup.document.write(buildQuestionBankPrintDocument(items, title, branding));
